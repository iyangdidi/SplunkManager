...
Core4j.toolbox.TableTree4j = function (configObject) {
    ...
    var canToggle = configObject.canToggle;
    if (canToggle == null) { canToggle = true; }
    ...
    this.useOrderReversal = configObject.useOrderReversal; //||false;
    if (this.useOrderReversal == null) { this.useOrderReversal = false; }
    ...
    this.onClickNodeEvents = configObject.onClickNodeEvents || [];
    this.onDblClickNodeEvents = configObject.onDblClickNodeEvents || [];
    this.eventsType = {
        ...
        ClickNodeEvents: "ClickNodeEvents",
        DblClickNodeEvents: "DblClickNodeEvents"
    };
    ...
    * */
    ...
    this.makeEachRowClassName = function (clsName) {
        return this.themeName + "-" + clsName + "-rowtr";
    };

    this.makeEachRowTDClassName = function (clsName) {
        return this.themeName + "-" + clsName + "-rowtd";
    };
    ...
    //make the row td id
    this.makeRowTdId = function (nodeid) {
        return this.id + "_TD_" + nodeid;
    };
    ...
    */
    ...
    //API
    this.setCanToggle = function (isCanToggle) {
        canToggle = isCanToggle;
        if (canToggle == null) {
            canToggle = true;
        }
    };
    ...
    //API
    this.reFreshTableTreeView = function (isReRenderNode) {
        ...
        this.initTreebuildRowsByNodes(this.rootNodes, true, isReRenderNode);
    ...
    */
    ...
    this.build = function (nodes, isJsondata) {
        ...
        if (nodes != null && nodes.length > 0) {
            ...
            this.initTreebuildRowsByNodes(this.rootNodes, false, true);
        }
        this.buildFooters();
    ...
    this.initTreebuildRowsByNodes = function (nodes, isReFreshTableTreeView, isReRenderNode) {
        //alert(nodes.length);
        if (nodes != null && nodes.length > 0) {
            ...
            for (var i = 0, n = nodes.length; i < n; i++) {
                ...
                this.addRowToTableObject(tmpNode, null, isReRenderNode);
                this.initTreebuildRowsByNodes(tmpNode.childs, isReFreshTableTreeView, isReRenderNode);
            }
        }
    };
    ...
    this.addRowToTableObject = function (node, index, isReRenderNode) {

        if (isReRenderNode == true) {
            node.dateValueEls = [];
        }
        ...
        //add class to the tr
        //2011-9-29
        Core4j.Domhelper.setElAttributeValue(rowObject, "class", this.makeEachRowClassName("row-base-cls"));
        ...
        Core4j.Domhelper.addEventToEl(rowObject, Core4j.Domhelper.ElEventType.CLICK, function () {
            ...
            if (tabletree.onClickNodeEvents != null) {
                for (var i = 0, n = tabletree.onClickNodeEvents.length; i < n; i++) {
                    var onClickNodeEvent = tabletree.onClickNodeEvents[i];
                    if (onClickNodeEvent != null && typeof (onClickNodeEvent) == 'function') {
                        goon = onClickNodeEvent([node], tabletree);
                        if (goon != null && goon == false) {
                            break;
                        }
                    }
                }
            }
            if (goon == null || goon == true) {
                var selectnodeindex = selectNodes.indexOf(node);
                // 没有选中的话
                if (selectnodeindex == -1) {
                    ...
                    //已经选中的话
                    tabletree.disSelectNode(node);
                }
            }
        });


        Core4j.Domhelper.addEventToEl(rowObject, 'dblclick', function () {

            var goon = null;

            if (tabletree.onDblClickNodeEvents != null) {
                for (var i = 0, n = tabletree.onDblClickNodeEvents.length; i < n; i++) {
                    var onDblClickNodeEvent = tabletree.onDblClickNodeEvents[i];
                    if (onDblClickNodeEvent != null && typeof (onDblClickNodeEvent) == 'function') {
                        goon = onDblClickNodeEvent(node, tabletree);
                        if (goon != null && goon == false) {
                            break;
                        }
                    }
        ...
        for (var i = 0, n = this.columns.length; i < n; i++) {
            ...
            //add class and id to the td
            //2011-9-30
            cellObject.setAttribute("id", this.makeRowTdId(node.id));
            Core4j.Domhelper.setElAttributeValue(cellObject, "class", this.makeEachRowTDClassName("rowtd-base-cls"));
...
//                Core4j.Domhelper.addEventToEl(iconDiv, Core4j.Domhelper.ElEventType.CLICK, function (event) {
//                    if (event.stopPropagation) {
//                        event.stopPropagation();
//                    } else {
//                        event.cancelBubble = true;
...
//                });
            ...
            if (isReRenderNode == true || node.dateValueEls[i] == null) {
                dateValueEl = Core4j.Domhelper.createElement('div', {
                    attributeNames: ['id', 'classStyle'],
                    valueObject: { id: this.makeValueDivId(node.id, i, isNodeClick), classStyle: contentValueClassName }
                }, cellObject);
                node.dateValueEls[i] = dateValueEl;
                /*if(isNodeClick){
                ...
                dateValueEl = node.dateValueEls[i];
                Core4j.Domhelper.setClass(dateValueEl, this.makeContentClassName("tt-node-click-content"));
                cellObject.appendChild(dateValueEl);
            }
    ...
    */
    ...
    this.configDefaultValueToTableTreeNodeBoject = function (node) {
        ...
        if (node.dateValueEls == null) { node.dateValueEls = []; }
    };
